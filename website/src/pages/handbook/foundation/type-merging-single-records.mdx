...
This example explores the core techniques for merging typed objects across subschemas using single-record queries, covering most of the topics discussed in the [documented basic example](/docs/approaches/type-merging#basic-example).

Using single-record queries means that every record accessed requires a _dedicated_ subschema delegation, versus a single delegation for an entire set of records. This 1:1 delegation strategy has far greater overhead than the array-batched technique discussed in the [merged arrays chapter](../type-merging-arrays). However&mdash;it's still useful when interacting with services beyond our control, and it's an excellent way to observe the behavior of [execution batching](/handbook/appendices/batching-arrays-and-queries#what-is-query-batching).

**This example demonstrates:**

- [One-way type merging](/docs/approaches/type-merging#basic-example) using single-record queries.
- [Multi-directional type merging](/docs/approaches/type-merging#unidirectional-merges) using single-record queries.
- Query/execution batching.

**Related examples:**

- See [array-batched type merging](/handbook/foundation/type-merging-arrays) to improve the performance of these basic patterns.
- See [stitching directives SDL](/handbook/foundation/stitching-directives-sdl) to write this merge configuration as schema directives.
...
<iframe
  src="https://codesandbox.io/embed/github/ardatan/schema-stitching/tree/master/examples/type-merging-single-records?codemirror=1&fontsize=14&initialpath=/graphql&theme=dark"
...
> You can also see the project on GitHub [here](https://github.com/ardatan/schema-stitching/tree/master/examples/type-merging-single-records).
...
Now we're sending a **single request** that resolves all three single-record queries at once, courtesy of [batched execution](/handbook/appendices/batching-arrays-and-queries#what-is-query-batching). There are very few reasons NOT to enable this free batching optimization (it will be enabled by default in the future).

Batch execution is superb for optimizing the exchange with the subservices. However, there are still overhead processing costs on the gateway for delegating each record individually, so batch execution alone is not a perfect solution. The best optimization strategy is to pair batch execution with delegating [arrays of records](/handbook/foundation/type-merging-arrays) at a time.
