...
> You can also see the project on GitHub [here](https://github.com/ardatan/schema-stitching/tree/master/examples/continuous-integration-testing).
...
2. The `schema_builder.ts` code builds a stitched schema used by the production server _and the test suite_. In this example, subschema configs are built separately from the stitched schema; this allows tests to build subschema configs, adjust them for testing, and then build them into the stitched schema.
