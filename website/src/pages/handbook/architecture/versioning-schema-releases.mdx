...
Of course, this sort of reload trigger **should not** be exposed in public APIs. You should [remove this sort of utility](../public-and-private-apis) from public-facing schemas, or use a separate non-GraphQL endpoint (such as a REST service) to trigger the action.

## Summary

There's [a lot to be said](https://web.archive.org/web/20201205030937/https://www.apollographql.com/docs/federation/managed-federation/advanced-topics/) about schema versioning and release strategies. Let's be (fairly) brief and distill a few key points:
