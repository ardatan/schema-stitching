...
See related [handbook example](/handbook/foundation/type-merging-single-records) for a working demonstration of this setup. This JavaScript-based syntax may also be written directly into schema-type definitions using the `@merge` directive of the [stitching SDL](/docs/approaches/directives-sdl):
...
The [basic example](/docs/getting-started/basic-example) above queries for a single record each time it performs a merge, which is suboptimal when merging arrays of objects. Instead, we should batch many record requests together using array queries that may fetch many partials at once, the schema for which would be:
...
A `valuesFromResults` method may also be provided to map the raw query result into the batched set. With this array optimization in place, we'll now only perform one query _per merged field_ (versus per record). However, requesting multiple merged fields will still perform a query each. To optimize this further, we can enable [query batching](/handbook/appendices/batching-arrays-and-queries#what-is-query-batching):
...
Using both array batching and query batching together is recommended and should flatten transactional costs down to one query per subservice per generation of data. See related [handbook example](/handbook/foundation/type-merging-arrays) for a working demonstration of this process.
...
Stubbed types are quick and easy to setup and effectively work as automatic [schema extensions](/docs/approaches/schema-extensions) (in fact, you might not need extensions). A stubbed type may always be expanded with additional service-specific fields (see the [basic example](/docs/getting-started/basic-example)). However, it requires a query in `merge` config as soon as it offers unique data.
...
In the above, both `Post` and `Section` will have a common interface of `{ id title url }` in the gateway schema. The difference in interface fields between the gateway schema and the layouts subschema will automatically be expanded into typed fragments for compatibility. See related [handbook example](/handbook/foundation/type-merging-interfaces) for a working demonstration.
...
In the above, the `shippingEstimate` and `deliveryService` fields are marked with `@computed` directives (see [stitching directives SDL](/docs/approaches/directives-sdl)), which specify additional _field-level dependencies_ required to resolve these specific fields beyond the `Product` type's base selection set. When a computed field appears in a query, the gateway will collect that field's dependencies from other subschemas, so they may be sent as input with the request for the computed field(s).
...
The main disadvantage of computed fields is that they cannot be resolved independently from the stitched gateway. Tolerance for this subservice inconsistency is largely dependent on your own service architecture. An imperfect solution is to deprecate all computed fields within a subschema, and then normalize their behavior in the gateway schema with a [`RemoveObjectFieldDeprecations`](/docs/transforms/grooming) transform. See related [handbook example](/handbook/foundation/computed-fields).
...
The following example uses [stitching directives](/docs/approaches/directives-sdl) to mark preferred subschema elements as `@canonical`:
...
<Callout type="warning">
  **Implementation note:** canonical settings are _only_ used for building the combined gateway schema definition and
  ...
  using [`typeMergingOptions`](/docs/approaches/combining-schemas#automatic-merge).
</Callout>
...
When incorporating plain objects, always extend the provided `originalObject` to retain the internal merge configuration. You may also return direct calls to `delegateToSchema` and `batchDelegateToSchema` (as described for [schema extensions](/docs/approaches/schema-extensions#basic-example)), however&mdash;always provide these delegation methods with a `skipTypeMerging: true` option to prevent infinite recursion.
