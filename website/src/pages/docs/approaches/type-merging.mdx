...
[stitching SDL](/docs/approaches/directives-sdl):
...
directives (see [stitching directives SDL](/docs/approaches/directives-sdl)), which specify
...
The following example uses [stitching directives](/docs/approaches/directives-sdl) to mark preferred
subschema elements as `@canonical`:
...
<Callout type="warning">
  **Implementation note:** canonical settings are _only_ used for building the combined gateway
  ...
  [`typeMergingOptions`](/docs/approaches/combining-schemas#automatic-merge).
</Callout>
