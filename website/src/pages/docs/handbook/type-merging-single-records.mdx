...
- [One-way type merging](/docs/approaches/type-merging#unidirectional-merges) using single-record queries.
- [Multi-directional type merging](/docs/approaches/type-merging#basic-example) using single-record queries.
...
Now we're sending a **single request** that resolves all three single-record queries at once, courtesy of [batched execution](/docs/handbook/batching-arrays-and-queries#what-is-query-batching). There are very few reasons NOT to enable this free batching optimization (it will be enabled by default in the future).
