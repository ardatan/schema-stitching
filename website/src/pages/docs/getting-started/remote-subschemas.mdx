...
- `executor`: is a generic method that performs requests to a remote schema. It's quite simple to [write your own](#executors). Subschema config uses the executor for query and mutation operations. See [handbook example](/handbook/foundation/combining-local-and-remote-schemas).
...
> You can see [this example](/handbook/foundation/type-merging-single-records#summary) that demonstrates the difference between batch execution and regular execution
